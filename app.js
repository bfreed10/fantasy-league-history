  ...
  $('#content').innerHTML=`
    ...
    <div class="grid-2 home-bottom-grid">
      ...
      <div class="card">
        ...
        ${franchises.map((f,i)=>`<div class="bar-row home-win-row">
...
function tradeAssetMarkup(assets,names=[]){
  const xs=(assets&&assets.length)?assets:(names||[]).map(Player=>({Player}));
  if(!xs.length) return '<span class="muted">No player detail recovered</span>';
  return `<div class="trade-assets">${xs.map(a=>{
    const actual=a.ActualROS==null?'—':fmt(a.ActualROS,1);
    const expected=a.ExpectedROS==null?'—':fmt(a.ExpectedROS,1);
    const surplus=a.Surplus==null?'':`<span class="trade-asset-surplus ${Number(a.Surplus)>=0?'good':'bad'}">${signed(a.Surplus,1)} vs exp.</span>`;
    return `<div class="trade-asset"><strong>${esc(a.Player||a.Name||`Player ${a.PlayerId||''}`)}</strong><span>${actual} actual ROS • ${expected} expected ROS</span>${surplus}</div>`;
  }).join('')}</div>`;
}
function tradeSideCell(x,side){
  const team=x[`Team${side}`]||`Team ${x[`Team${side}Id`]||''}`;
  const owner=x[`Team${side}Owner`]||'';
  const assets=x[`Team${side}Assets`]||[];
  const names=x[`Team${side}Players`]||[];
  const actual=x[`Team${side}ActualROS`];
  const expected=x[`Team${side}ExpectedROS`]!=null?x[`Team${side}ExpectedROS`]:x[`Team${side}ExpectedROSModel`];
  const surplus=x[`Team${side}Surplus`];
  return `<div class="trade-side"><div class="trade-side-head"><div><strong>${esc(team)}</strong><span>${esc(canonicalOwner(owner))}</span></div><div class="trade-side-total"><strong>${actual==null?'—':fmt(actual,1)}</strong><span>ROS pts</span></div></div>${tradeAssetMarkup(assets,names)}<div class="trade-side-summary"><span>Expected ${expected==null?'—':fmt(expected,1)}</span>${surplus==null?'':`<span class="${Number(surplus)>=0?'good':'bad'}">${signed(surplus,1)} surplus</span>`}</div></div>`;
}
function tradeWhen(x){
  const w=Number(x.Week||0); return `${x.Season}${w?` W${w}`:''}`;
}

function renderTrades(){
  setHeader('Transactions & Trades','Historical roster activity, official ESPN trade counts, and validated player-for-player trade grading.');
  ...
  const drawTab=tab=>{
    ...
    if(tab==='trades'){
      if(recovered.length){
        const flea=(A.biggestFleeces||[]).slice(0,20), balanced=(A.mostBalanced||[]).slice(0,20), career=A.managerCareer||[];
        const verifiedYears=A.verifiedSeasons||[...new Set(recovered.map(x=>Number(x.Season)))].sort((a,b)=>b-a);
        const tradeTeams=[...new Map(recovered.flatMap(x=>['A','B'].map(side=>({id:Number(x[`Team${side}Id`]),team:x[`Team${side}`],owner:x[`Team${side}Owner`]}))).filter(x=>x.id).map(x=>[x.id,x])).values()].sort((a,b)=>String(a.team).localeCompare(String(b.team)));
        const swingVal=x=>x.Swing!=null?Number(x.Swing):x.SurplusGap!=null?Number(x.SurplusGap):Number(x.ValueGap||0);
        const tradeRow=x=>`<tr>
          <td><strong>${tradeWhen(x)}</strong><br><span class="muted">${x.Date?esc(String(x.Date).slice(0,10)):''}</span></td>
          <td>${tradeSideCell(x,'A')}</td>
          <td>${tradeSideCell(x,'B')}</td>
          <td><strong>${fmt(x.ValueGap||0,1)}</strong><br><span class="muted">actual gap</span></td>
          <td>${x.ExpectedGap==null?'—':`${fmt(x.ExpectedGap,1)}<br><span class="muted">expected gap</span>`}</td>
          <td class="${valueTone(swingVal(x))}"><strong>${fmt(swingVal(x),1)}</strong><br><span class="muted">vs expectation</span></td>
        </tr>`;
        const careerRows=career.map((x,i)=>`<tr>
          <td>${i+1}</td>
          <td>${displayFranchise(x.TeamID,x.Team,x.Manager||x.Owner)}</td>
          <td><strong>${x.Wins}-${x.Losses}${x.Ties?`-${x.Ties}`:''}</strong></td>
          <td>${fmt(x.Trades)}</td>
          <td>${x.WinPct==null?'—':fmt(Number(x.WinPct)*100,1)+'%'}</td>
          <td class="${valueTone(x.NetActualValue)}">${signed(x.NetActualValue,1)}</td>
          <td class="${valueTone(x.NetSwing)}"><strong>${signed(x.NetSwing,1)}</strong></td>
        </tr>`);
        panel.innerHTML=`
          <div class="metrics">
            ${metric('Verified packages',recovered.length,`${verifiedYears.length} checksum-verified season${verifiedYears.length===1?'':'s'}`)}
            ${metric('Official league trades',tradeCountText(T.completedTrades),'all seasons • counter total')}
            ${metric('Biggest expectation swing',flea[0]?fmt(swingVal(flea[0]),1):'—','points')}
            ${metric('Franchises graded',career.length,'verified package subset')}
          </div>
          <div class="trade-method-note">
            <span class="badge good">No proposal pollution</span>
            <div><strong>How grading works</strong><span class="muted">Only ESPN completed-trade activity is used. Actual ROS = fantasy points from the trade scoring period forward. Expected ROS = ESPN weekly projections over the same remaining periods where available. “Swing” measures how much the actual side-to-side differential moved versus the expected differential.</span></div>
          </div>
          <div class="grid-2 section-gap">
            <div class="card"><h2>Biggest Fleeces vs Expectation</h2><p class="muted">Largest absolute swing from what ESPN projections expected at the player/week level.</p>${table(['When','Side A Received','Side B Received','Actual Gap','Expected Gap','Swing'],flea.slice(0,10).map(tradeRow))}</div>
            <div class="card"><h2>Most Balanced Trades</h2><p class="muted">Smallest realized rest-of-season point gap between the two packages.</p>${table(['When','Side A Received','Side B Received','Actual Gap','Expected Gap','Swing'],balanced.slice(0,10).map(tradeRow))}</div>
          </div>
          <div class="card section-gap">
            <div class="card-heading-row"><div><span class="section-eyebrow">CAREER TRADE GRADES</span><h2>Franchise Trading Record</h2></div><span class="muted">verified packages only</span></div>
            ${table(['#','Franchise / Manager','Record','Trades','Win %','Net Actual','Net Swing'],careerRows)}
          </div>
          <div class="card section-gap">
            <div class="card-heading-row"><div><span class="section-eyebrow">SEARCHABLE ARCHIVE</span><h2>Recovered Trade Timeline</h2></div><span class="muted" id="tradeResultCount"></span></div>
            <div class="controls trade-controls">
              <label>Season <select id="tradeSeason"><option value="">All verified</option>${verifiedYears.map(y=>`<option value="${y}">${y}</option>`).join('')}</select></label>
              <label>Franchise <select id="tradeManager"><option value="">All franchises</option>${tradeTeams.map(x=>`<option value="${x.id}">${esc(x.team)} — ${esc(canonicalOwner(x.owner))}</option>`).join('')}</select></label>
              <label>Player search <input id="tradeSearch" type="search" placeholder="e.g. Derrick Henry"></label>
            </div>
            <div id="tradeTimeline"></div>
          </div>`;
        const drawTrades=()=>{
          const yr=Number($('#tradeSeason').value||0), tid=Number($('#tradeManager').value||0), q=String($('#tradeSearch').value||'').trim().toLowerCase();
          const rows=recovered.filter(x=>{
            if(yr&&Number(x.Season)!==yr)return false;
            if(tid&&Number(x.TeamAId)!==tid&&Number(x.TeamBId)!==tid)return false;
            if(q){const hay=[...(x.TeamAPlayers||[]),...(x.TeamBPlayers||[]),x.TeamA,x.TeamB,x.TeamAOwner,x.TeamBOwner].join(' ').toLowerCase();if(!hay.includes(q))return false;}
            return true;
          }).slice().sort((a,b)=>Number(b.Timestamp||0)-Number(a.Timestamp||0));
          $('#tradeResultCount').textContent=`${rows.length} trade${rows.length===1?'':'s'}`;
          $('#tradeTimeline').innerHTML=rows.length?table(['When','Side A Received','Side B Received','Actual Gap','Expected Gap','Swing'],rows.map(tradeRow)):'<div class="empty-state">No verified trades match those filters.</div>';
        };
        $('#tradeSeason').addEventListener('change',drawTrades);$('#tradeManager').addEventListener('change',drawTrades);$('#tradeSearch').addEventListener('input',drawTrades);drawTrades();
      }else{
        const yearRows=seasons.map(x=>`<tr><td><strong>${x.Season}</strong></td><td>${fmt(x.TradeParticipations)}</td><td><strong>${tradeCountText(x.CompletedTrades)}</strong></td><td><span class="badge good">Count verified</span></td><td><span class="badge warn">Run v10 enricher</span></td></tr>`);
        ...
        panel.innerHTML=`
          <div class="trade-recovery-banner">
            <div>
              <span class="section-eyebrow">TRADE CENTER V10</span>
              <h2>The page is ready for package-level grading.</h2>
              <p>The site still shows only verified counts until the local v10 enricher runs with your private ESPN cookies. The enricher uses completed-trade activity, checks every recovered season against ESPN's saved franchise trade counters, and publishes only seasons that match exactly.</p>
            </div>
            <div class="trade-recovery-number"><strong>${tradeCountText(T.completedTrades)}</strong><span>verified league trades</span></div>
          </div>
          <div class="grid-2">
            <div class="card"><h2>Verified Trade Volume by Season</h2>${table(['Season','Team Participations','Completed Trades','Count Status','Package Status'],yearRows)}</div>
            <div class="card"><h2>Most Frequent Traders</h2>${table(['#','Franchise / Manager','Seasons','Trade Parts.','Per Season'],tradeFranchises)}</div>
          </div>
          <div class="identity-note section-gap"><span class="badge good">Safe by default</span><div><strong>No fabricated trade packages</strong><span class="muted">If a season does not reconcile exactly, its individual deals stay hidden while the official aggregate counts remain visible.</span></div></div>`;
      }
...
function renderData(){
  ...
  <div class="hero" style="margin-top:16px"><h2>Provenance first</h2><p>The site keeps raw ESPN history intact and applies identity cleanup as a transparent presentation layer. Historical acquisition, drop and trade-count summaries come from saved ESPN counters. Individual trade packages are shown only for seasons that pass the same team-level checksum; realized trade grades use actual rest-of-season scoring versus ESPN expected rest-of-season scoring where available.</p></div>`;
}
